...
export default function Layout({ children }) {
  ...
  }, [location.pathname, user?.role, user?.userId, user?.userID, user?.id, fetchHeaderNotifications, fetchPendingTasksCount]);
  ...
  return (
    <div className="layout-container">
      ...
      <div className="layout-main">
        ...
        {/* Page content */}
        {alert?.visible && (
          <div className={`global-alert global-alert-${alert.type || 'info'}`} role="status">
            <div className="global-alert-message">{alert.message}</div>
            <button className="global-alert-close" onClick={clearAlert} aria-label="Dismiss notification">✕</button>
          </div>
        )}
