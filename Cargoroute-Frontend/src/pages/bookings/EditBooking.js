...
import { getBookingById, updateBooking, getAllShippers } from '../../api/bookingsApi';
...
export default function EditBooking() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([bookingData, shipperData]) => {
        setShippers(shipperData);
        // Parse special handling flags from comma-separated string
        const flags = bookingData.specialHandlingFlags
          ? bookingData.specialHandlingFlags.split(',').filter(Boolean)
          : [];
        setForm({
          shipperID: bookingData.shipper?.shipperID || '',
          originSiteID: bookingData.originSiteID || '',
          destinationSiteID: bookingData.destinationSiteID || '',
          pickupWindowStart: formatDateTime(bookingData.pickupWindowStart),
          pickupWindowEnd: formatDateTime(bookingData.pickupWindowEnd),
          deliveryWindowStart: formatDateTime(bookingData.deliveryWindowStart),
          deliveryWindowEnd: formatDateTime(bookingData.deliveryWindowEnd),
          weightKg: bookingData.weightKg || '',
          volumeM3: bookingData.volumeM3 || '',
          pieces: bookingData.pieces || '',
          commodity: bookingData.commodity || '',
          specialHandlingFlags: flags,
  ...
  // Convert date-time string to datetime-local format
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Booking updated successfully! Redirecting…' });
      setTimeout(() => navigate('/bookings'), 1500);
    } catch (err) {
      setMessage({ type: 'error', text: err.response?.data?.message || 'Failed to update booking.' });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) return <Layout><div className="empty-state">Loading booking…</div></Layout>;
      ...
      <div className="booking-form-page">
        <div className="form-page-header">
          ...
          <div>
            ...
            <p className="page-subtitle">{formatBookingId(id)}</p>
          </div>
        </div>
